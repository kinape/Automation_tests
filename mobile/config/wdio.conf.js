...
exports.config = {
    ...
    // Desabilita animações e fecha diálogos do sistema antes das suítes
    beforeSuite: function () {
        try {
            const adbArgsBase = [];
            if (udid) adbArgsBase.push('-s', udid);
            cp.spawnSync('adb', [...adbArgsBase, 'shell', 'am', 'broadcast', '-a', 'android.intent.action.CLOSE_SYSTEM_DIALOGS'], { shell: true, encoding: 'utf8' });
            const animCmds = [
                ['shell','settings','put','global','window_animation_scale','0'],
                ['shell','settings','put','global','transition_animation_scale','0'],
                ['shell','settings','put','global','animator_duration_scale','0'],
            ];
            for (const args of animCmds) {
                cp.spawnSync('adb', [...adbArgsBase, ...args], { shell: true, encoding: 'utf8' });
            }
        } catch {}
    },
    autoCompileOpts: {
